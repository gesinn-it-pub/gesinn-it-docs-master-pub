...
Parser functions, tag hooks, and result printers can return `[ $text, 'isHTML' => true ]` (or set `$parser->getOutput()`/`ParserOutput::setText()` equivalents) to tell MediaWiki the string is already-final HTML.
This is a stronger claim than `Html::rawElement()`'s "this fragment is safe": it tells the parser to skip wikitext processing for the *entire* returned value, via a strip marker that is spliced back in verbatim near the end of parsing.
Nothing sanitizes an `isHTML` string afterwards — if it contains unparsed wikitext or a user-controlled value, that reaches the page as-is.

This means every conditional branch that can produce the returned string must independently guarantee it is fully resolved to HTML.
A guard that only sometimes runs the resolving step is not a performance shortcut — it is an unparsed-output leak, and if any value it touches is user-authored (a template parameter, a query parameter such as a search/caption label, wikitext from a transcluded page), it is stored or reflected XSS.
...
Before writing an ad-hoc call to `Parser::recursiveTagParse()` (which requires an in-progress parse — it fatals otherwise) or a bare `ParserFactory::getMainInstance()`, check whether the repository already provides a text-processing helper for this exact situation.
Frameworks that inject query results into result printers commonly hand the printer the *in-flight* parser together with a wrapper that checks whether a parse is actually in progress and falls back to a full parse otherwise (for example, `RecursiveTextProcessor` in Semantic MediaWiki-based extensions, injected by the query processor).
Reusing that wrapper is both safer (no unparsed branch, no wrong-parser-instance bugs during nested parses) and consistent with how the rest of the codebase already does this — grep the codebase for how sibling result printers/classes build similar links before inventing a new approach.
...
Use BEM (Block\__Element\--Modifier) with the extension-specific prefix defined in `css.adoc`.
...
*Regression tests for an escaping fix — assert both directions*

A test that only checks the escaped form is present can pass for the wrong reason (e.g. the payload happened to contain no special characters that round-trip differently). A test that only checks the raw payload is absent can pass because the output was mangled some other way, not properly escaped. An escaping regression test asserts both in the same case:

[source,text]
----
assert-output:
  to-contain:
    - "&lt;img src=x onerror=alert() &gt;"   // the escaped form is present
  not-contain:
    - "<img src=x onerror=alert() >"          // the raw payload never reached output
----

Cover every output mode/sink the value can reach independently (e.g. inline wikitext vs. a raw/API/export mode that bypasses the parser) — a sink that is parser-sanitised today is not evidence that a sibling sink bypassing the parser is also safe. When auditing a reflected-input surface (a query parameter, a pagination token, a form field echoed back into an error message), check every branch that renders that value, not just the one the report described.
